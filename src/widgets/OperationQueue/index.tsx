...
function ({ toaster, children }) {
  ...
  const ops = [_lockingOperationKey, ...Object.entries(_opKeys).flat()];

  const notify = useCallback(function _notify(opKey: string) {
    const opCount =  _opKeys.get(opKey) ?? 0;

    if (opCount < 1) {
      toaster.show({ message: `Done ${opKey}`, intent: 'success', icon: 'tick-circle' }, opKey);
    } else {
      toaster.show({
  ...
  const unqueue = useCallback(function _unqueue(opKey: string, isBlocking: boolean) {
    ...
    setOpKeys(_opKeys.set(opKey, (_opKeys.get(opKey) ?? 1) - 1));
    notify(opKey);
  }, ops);
  ...
  const queue = useCallback(function _queue(opKey: string, isBlocking: boolean) {
    ...
    setOpKeys(_opKeys.set(opKey, (_opKeys.get(opKey) ?? 0) + 1));
    notify(opKey);
  }, ops);
